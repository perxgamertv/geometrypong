...
//		mypart=GameObject.Find("MyPart").GetComponent<ParticleSystem>();
	...
	void OnCollisionEnter(Collision other){
		if(other.gameObject.name=="PontoP1"){
			GameObject.Find("SpawnBola").GetComponent<SpawnBola>().CmdPontoP1();
			Destroy(transform.gameObject);
		}
			if(other.gameObject.name=="PontoP2"){
			GameObject.Find("SpawnBola").GetComponent<SpawnBola>().CmdPontoP2();
			Destroy(transform.gameObject);
		}
	...
	[Command]
	void CmdInstance(){
		spawngo = Instantiate(mypart,this.transform.position,this.transform.rotation);
